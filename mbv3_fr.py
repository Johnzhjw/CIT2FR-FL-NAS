...
# from layers import *
from ofa.layers import MBInvertedConvLayer, ConvLayer, IdentityLayer, LinearLayer
from layer_FR import set_layer_from_config, FuzzyLayer
...
class MobileNetV3(MyNetwork):
    ...
    def __init__(self, first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier):
        super(MobileNetV3, self).__init__()

        self.first_conv = first_conv
        self.blocks = nn.ModuleList(blocks)
        self.final_expand_layer = final_expand_layer
        self.fuzzy_layer = fuzzy_layer
    ...
    def build_from_config(config):
        ...
        fuzzy_layer = set_layer_from_config(config['fuzzy_layer'])
        ...
        net = MobileNetV3(first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier)
        if 'bn' in config:
    ...
    def build_net_via_cfg(cfg, input_channel, last_channel, n_classes, dropout_rate):
        ...
        fuzzy_layer = FuzzyLayer(in_channels=feature_dim)
        # classifier
        classifier = LinearLayer(last_channel, n_classes, dropout_rate=dropout_rate)

        return first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier
...
class MobileNetV3Large(MobileNetV3):
    ...
    def __init__(self, n_classes=1000, width_mult=1, bn_param=(0.1, 1e-3), dropout_rate=0.2,
        ...
        first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier = self.build_net_via_cfg(
            cfg, input_channel, last_channel, n_classes, dropout_rate
        )
        super(MobileNetV3Large, self).__init__(first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier)
        # set bn param
