...
class OFAEvaluator:
    ...
    @staticmethod
    def eval(subnet, data_path, dataset='imagenet', n_classes=1000,
             n_epochs=0, resolution=224, trn_batch_size=128, vld_batch_size=250,
        ...
        run_config = get_run_config(
            ...
            n_worker=num_workers, valid_size=valid_size, flag_FL=cfgs.flag_FL, size_FL=cfgs.size_FL)
        ...
        if n_epochs > 0:
            ...
            NSGANetV2.reset_classifier(
                subnet,
                last_channel=subnet.classifier.in_features,
                n_classes=n_classes, dropout_rate=cfgs.drop_rate)
        ...
        loss, top1, top5 = [], [], []
        t_loss, t_top1, t_top5 = run_manager.validate(net=subnet, is_test=is_test, no_logs=False)
        loss.append(t_loss)
        top1.append(t_top1)
        top5.append(t_top5)
        if not is_test:
            t_loss, t_top1, t_top5 = run_manager.validate(net=subnet, is_test=True, no_logs=False)
            loss.append(t_loss)
            top1.append(t_top1)
            top5.append(t_top5)
...
def main(args):
    ...
    if mode == 'config':
        ...
        evaluator = OFAEvaluator(n_classes=args.n_classes, model_path=args.supernet_path, flag_fuzzy=args.flag_fuzzy)
    ...
    OFAEvaluator.eval(
        subnet, log_dir=args.log_dir, data_path=args.data, dataset=args.dataset, n_classes=args.n_classes,
