...
from layer_FR import FuzzyLayer
from mbv3_fr import MobileNetV3, MobileInvertedResidualBlock
...
class OFAMobileNetV3(MobileNetV3):
    ...
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, base_stage_width=None,
        ...
        # base_stage_width = [16, 24, 32, 40, 48, 64, 128, 64]
        base_stage_width = [16, 32, 64, 128, 64]

        final_expand_width = [
            make_divisible(base_stage_width[-2] * max(self.width_mult_list), 8) for _ in self.width_mult_list
        ]
        if flag_fuzzy:
            tmp_ch = 160 if base_stage_width[-1] > 160 else base_stage_width[-1]
            last_channel = [
                make_divisible(tmp_ch * max(self.width_mult_list), 8) for _ in self.width_mult_list
            ]
        else:
            last_channel = [
        ...
        act_stages = ['relu', 'relu', 'h_swish']
        # se_stages = [False, False, True, False, True, True]
        se_stages = [False, True, True]
        ...
        fuzzy_layer = FuzzyLayer(in_channels=max(final_expand_width), flag_layer=flag_fuzzy)
        ...
        super(OFAMobileNetV3, self).__init__(first_conv, blocks, final_expand_layer,
                                             fuzzy_layer, feature_mix_layer, classifier)
    ...
    def forward(self, x):
        ...
        if self.fuzzy_layer.get_flag:
            x = self.fuzzy_layer(x).squeeze(-1)
        else:
    ...
    def module_str(self):
        ...
        _str += self.fuzzy_layer.module_str + '\n'
    ...
    def config(self):
        return {
            ...
            'fuzzy_layer': self.fuzzy_layer.config,
    ...
    def load_weights_from_net(self, src_model_dict):
        ...
        for key in src_model_dict:
            ...
            elif '.fuzzy.fuzzy.' in key:
                new_key = key.replace('.fuzzy.fuzzy.', '.fuzzy.')
            ##############################################################################
            elif '.linear.' in key:
                new_key = key.replace('.linear.', '.linear.linear.')
            elif '.fuzzy.' in key:
    ...
    def set_active_subnet(self, wid=None, ks=None, e=None, d=None, f=None):
        ...
        if f is not None:
            if f:
                self.fuzzy_layer.get_flag = True
            else:
                self.fuzzy_layer.get_flag = False
    ...
    def sample_active_subnet(self):
        ...
        # fuzzy flag
        fuzzy_setting = random.choice([0, 1])

        self.set_active_subnet(width_mult_setting, ks_setting, expand_setting, depth_setting, fuzzy_setting)
    ...
    def get_active_subnet(self, preserve_weight=True):
        ...
        fuzzy_layer = copy.deepcopy(self.fuzzy_layer)
        ...
        _subnet = MobileNetV3(first_conv, blocks, final_expand_layer, fuzzy_layer, feature_mix_layer, classifier)
    ...
    def get_active_net_config(self):
        ...
        fuzzy_config = self.fuzzy_layer.config
