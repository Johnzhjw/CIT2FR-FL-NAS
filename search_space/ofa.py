...
class OFASearchSpace:
    ...
    def sample(self, n_samples=1, nb=None, ks=None, e=None, d=None, r=None, f=None):
        ...
        f = self.fuzzy if f is None else f
        ...
        for n in range(n_samples):
            ...
            fuzzy = int(np.random.choice(f))

            data.append({'ks': kernel_size, 'e': exp_ratio, 'd': depth, 'r': resolution, 'f': fuzzy})
        return data
    ...
    def initialize(self, n_doe):
        ...
        data = [
            self.sample(1, ks=[min(self.kernel_size)], e=[min(self.exp_ratio)],
                        d=[min(self.depth)], r=[min(self.resolution)], f=[min(self.fuzzy)])[0],
            self.sample(1, ks=[max(self.kernel_size)], e=[max(self.exp_ratio)],
                        d=[max(self.depth)], r=[max(self.resolution)], f=[max(self.fuzzy)])[0]
        ]
    ...
    def encode(self, config):
        ...
        x.append(np.argwhere(config['f'] == np.array(self.fuzzy))[0, 0])
    ...
    def decode(self, x):
        ...
        return {'ks': kernel_size, 'e': exp_rate, 'd': depth, 'r': self.resolution[x[-2]], 'f': self.fuzzy[x[-1]]}
